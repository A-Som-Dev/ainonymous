...
- Data at rest produced by AInonymity: JSONL audit files, the shutdown token file in the OS temp directory, the `.ainonymity.yml` configuration file.
- Local IPC surface: the HTTP listener on `127.0.0.1:8100` and the `/shutdown`, `/metrics`, `/dashboard`, `/events`, `/health` endpoints.
- The supply chain of the npm package `ainonymity` and its transitive dependencies.
- Behavior under `AINONYMITY_HOST=0.0.0.0` (container deployments), where the listener is exposed beyond loopback.

### Out of scope

- The upstream LLM provider. Once an anonymized request reaches `api.anthropic.com` or `api.openai.com`, the provider's own threat model and terms of service apply. AInonymity does not and cannot enforce anything past the TLS boundary.
...
- The user's own source code, prompts, or editor. If the user pastes an unredacted secret into a prompt that AInonymity has no pattern for, the secret leaves the machine. Detection coverage is best-effort (see Non-Mitigations).
- The LLM client tool (Claude Code, Cursor, Aider, Cody, Continue). AInonymity sets `ANTHROPIC_BASE_URL` / `OPENAI_BASE_URL` and trusts the tool to use them. A tool that bypasses those env vars for telemetry or analytics endpoints is not protected against.
...
|                                                                  |
|   +------------+        +-------------------------------+        |
|   | LLM client | --->   | ainonymity proxy (node:http)  |        |
...
|   - .ainonymity.yml                     |                        |
|   - ./ainonymity-audit/*.jsonl          |                        |
|   - $TMPDIR/ainonymity-<port>.token     |                        |
+-----------------------------------------+------------------------+
                                          |
                                          | public internet
                                          v
...
- **TB3** user <-> the `ainonymity` npm package and its transitive dependencies.
...
**Capability:** Read the process memory of the node runtime via `/proc/<pid>/maps` on Linux or `OpenProcess + ReadProcessMemory` on Windows. Read files the user owns, including `.ainonymity.yml`, the audit JSONL files and the token file in `$TMPDIR`. Connect to `127.0.0.1:8100`.
...
**Typical actions:**
- Read `/tmp/ainonymity-8100.token` (mode 0600 on POSIX, default ACL on Windows) and invoke the shutdown endpoint.
...
**Primary defense:** process isolation provided by the OS. AInonymity does not defend against same-UID attackers beyond the usual surface hardening.

### A2 - Kompromittierte Config

**Capability:** Can write to `.ainonymity.yml` in the project directory.
...
**Threat classification:** this is equivalent to a supply-chain attack on the configuration file. If the attacker can write `.ainonymity.yml`, they can also write `~/.npmrc`, `~/.bashrc`, or the client tool's own config. The proxy's confidentiality guarantees assume the config is trusted input.
...
**What AInonymity contributes:** the request body already has secrets redacted and identifiers replaced with pseudonyms. The high-value payload (original names, domains, code identifiers) is not on the wire. TLS verification itself is delegated to Node.js defaults; there is no certificate pinning.
...
**Motivation:** Exploit the rehydration step. If the upstream returns text containing a pseudonym the session map knows about, AInonymity substitutes the original. If it returns a pseudonym it does not know, rehydration is a no-op.
...
**Typical actions:**
- Typosquat `ainonymity` as `anonymity`, `ainonimity`, etc. A user who mistypes `npm install ainonymous` gets the attacker's package.
...
**What AInonymity contributes:** SBOM is generated (mentioned in `HANDOVER.md`). `npm audit` reports 0 vulnerabilities at release. Deep dependency tree is intentionally shallow; `node:http`/`node:https` are used instead of express/axios.
...
| STRIDE | Threat | Status | Notes |
|---|---|---|---|
| S | Attacker runs `ainonymous stop` without authorization. | `partial` | Token file is `$TMPDIR/ainonymity-<port>.token`, mode 0600 on POSIX. Windows default ACL is not guaranteed private (P5-14 flags this). Any same-UID process on POSIX can still read mode-0600 files. |
...
AInonymity does not protect against:
...
- The user's `.ainonymity.yml` is honest. A compromised config is treated as a compromised host.
- The upstream LLM provider handles the anonymized payload according to their terms. AInonymity does not inspect what the provider does with it.

---

## 8. Future Work

- **P1-1** ~~Auth on `/metrics`, `/dashboard`, `/events`.~~ Done: bearer token via `behavior.mgmt_token` / `AINONYMITY_MGMT_TOKEN`. See `SECURITY.md` → "Management endpoint auth".
...
- **P5-14** ~~Windows ACL for the shutdown token file.~~ Done: stored under `%USERPROFILE%\.ainonymity\` with best-effort icacls hardening.
