...
export class Pipeline {
  ...
  constructor(config: AInonymityConfig, logger?: AuditLogger) {
    ...
    if (persist && !sessionKey) {
      throw new Error(
        'session.persist is true but AINONYMITY_SESSION_KEY is not set (or invalid). ' +
...
function resolveSessionKey(): Buffer | undefined {
  const raw = process.env['AINONYMITY_SESSION_KEY'];
  if (!raw) return undefined;
  try {
    const buf = Buffer.from(raw, 'base64');
    if (buf.length !== 32) {
      log.warn('AINONYMITY_SESSION_KEY has wrong length, ignoring', { length: buf.length });
      return undefined;
    }
    return buf;
  } catch (err) {
    log.warn('AINONYMITY_SESSION_KEY could not be parsed, ignoring', {
