...
describe('AuditLogger export', () => {
  ...
  beforeEach(() => {
    logger = new AuditLogger();
    tmpDir = mkdtempSync(join(tmpdir(), 'ainonymity-test-'));
  });
...
describe('AuditLogger persistence', () => {
  ...
  beforeEach(() => {
    logger = new AuditLogger();
    tmpDir = mkdtempSync(join(tmpdir(), 'ainonymity-persist-'));
  });
  ...
  it('creates JSONL file on first log after enablePersistence', () => {
    ...
    const expectedFile = join(tmpDir, `ainonymity-audit-${stamp}.jsonl`);
  ...
  it('writes one JSON object per line', () => {
    ...
    const d = new Date();
    const stamp = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
    const filePath = join(tmpDir, `ainonymity-audit-${stamp}.jsonl`);
...
describe('AuditLogger hash chain', () => {
  ...
  beforeEach(() => {
    logger = new AuditLogger();
    tmpDir = mkdtempSync(join(tmpdir(), 'ainonymity-chain-'));
    logger.enablePersistence(tmpDir);
  });
  ...
  it('verifyAuditChain returns null for intact persisted log', () => {
    ...
    const d = new Date();
    const stamp = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
    const file = join(tmpDir, `ainonymity-audit-${stamp}.jsonl`);
